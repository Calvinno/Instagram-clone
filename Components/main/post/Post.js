...
function Post(props) {
    ...
    useEffect(() => {
        if (props.route.params.notification != undefined) {
            ...
            const postPath = `post/${props.route.params.user}/userPosts/${props.route.params.item}`;
            const postReference = doc(db, postPath);
            getDoc(postReference)
            .then((snapshot) => {
                if (snapshot.exists()) {
            ...
            const likesPath = `post/${props.route.params.user}/userPosts/${props.route.params.item}/likes/${auth.currentUser.uid}`;
            const likesReference = doc(db, likesPath);
            const listener = onSnapshot(likesReference, (snapshot) => {
            ...
            const likesPath = `post/${props.route.params.user.uid}/userPosts/${props.route.params.item.id}/likes/${auth.currentUser.uid}`;
            const likesReference = doc(db, likesPath);
            const listener = onSnapshot(likesReference, (snapshot) => {
                let currentUserLike = false;
                if (snapshot.exists()) {
    ...
    const onLikePress = (userId, postId, item) => {
        ...
        const likePath = `post/${userId}/userPosts/${postId}/likes/${auth.currentUser.uid}`;
        const likeReference = doc(db, likePath);
        setDoc(likeReference, {})
        props.sendNotification(user.notificationToken, "New Like", `${props.currentUser.name} liked your post`, { type: 0, postId, user: auth.currentUser.uid });
    }
    ...
    return (
        <View style={[container.container, utils.backgroundWhite]}>
            ...
            <View>
                ...
                <View style={[{flexDirection: 'column'}, utils.padding10Sides]}>
