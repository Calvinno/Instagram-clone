...
export default function CameraScreen(props) {
    ...
    if(cameraRef.current) {
      try {
        ...
        if (videoRecordPromise) {
          ...
          const source = data.uri;
          let imageSource = await generateThumbail(source)
  ...
  const generateThumbail = async (source) => {
    try {
  ...
  const handleGoToSaveOnGalleryPick = async () => {
    ...
    const loadAsset = await MediaLibrary.getAssetInfoAsync(galleryPickedImage);
    let imageSource = null
    if (type == 0) {
      imageSource = await generateThumbail(galleryPickedImage.uri)
    }

    props.navigation.navigate('Save', {
      source: loadAsset.localUri,
      type,
      imageSource
    })
  }
        ...
        <ScrollView
            ...
            <View
                ...
                <Image
                    ...
                    style={[{ aspectRatio: 1 / 1, height: WINDOW_WIDTH }, { flex: 1 }]}
                    ratio={'1:1'}
